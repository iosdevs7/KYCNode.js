...
var circumplexData = require('../json/traits.json');
var facetsData = require('../json/facets.json');
var valuesData = require('../json/values.json');
/*
$.ajax({
  dataType: 'json',
  type: 'GET',
...
module.exports.assembleTraits = function assembleTraits(personalityTree) {
  var sentences = [];
  var big5elements = [];
  ...
  switch (relevantBig5.length) {
    ...
    case 5:
      ...
      sentences.push('Your child is ' + adj1 + ', ' + adj2 + ' and ' + adj3 + '.');
      break;
  }
...
module.exports.assembleFacets = function assembleFacets(personalityTree) {
  var sentences = [];
  var facetElements = [];
  ...
  sentences.push('Your child is ' + info.term + ': ' + info.description + '.');
  ...
  sentences.push('And your child is ' + info.term + ': ' + info.description + '.');
 ...
 */
module.exports.assembleValues = function assembleValues(valuesTree) {
  var sentences = [];
  var valuesList = [];
  ...
  if (sameQI) {
    ...
    switch (intervalFor(valuesList[0].percentage)) {
      case 0:
        sentence = 'Your child is relatively unconcerned with both '.
        concat(info1.term).
        concat(' and ').
        concat(info2.term).
        concat('.');
        break;
      case 1:
        sentence = 'Your child doesn\'t find either '.
        concat(info1.term).
        concat(' or ').
        concat(info2.term).
        concat(' to be particularly motivating for him/her.');
        break;
      case 2:
        sentence = 'Your child values both '.
        concat(info1.term).
        concat(' and ').
        concat(info2.term).
        concat(' a bit.');
        break;
      case 3:
        sentence = 'Your child considers both '.
        concat(info1.term).
        concat(' and ').
        concat(info2.term).
        concat(' to guide a large part of what he/she does.');
        break;
    }
    ...
    for (var i = 0; i < valuesInfo.length; i++) {
      ...
      switch (intervalFor(valuesList[i].percentage)) {
        case 0:
          sentence = 'Your child is relatively unconcerned with '.
          ...
          break;
        case 1:
          sentence = 'Your child doesn\'t find '.
          concat(valuesInfo[i].term).
          concat(' to be particularly motivating for him/her: ').
          concat(valuesInfo[i].description.toLowerCase()).
          concat('.');
